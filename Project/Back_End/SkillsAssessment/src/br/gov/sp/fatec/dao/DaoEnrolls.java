...
*/	
	@SuppressWarnings("finally")
	public static boolean validate(Connection conn, String email, Long ra){
		String query = "SELECT IF( (SELECT count(*) FROM user "
									+ "WHERE usr_userName = ? OR usr_ra = ?) > 0,'TRUE','FALSE') AS BOOLEAN "
									+ "FROM dual;";
		...
		try{
			...
			ResultSet rs = stmt.executeQuery();
			if(rs.next()) {
				validate = rs.getBoolean("BOOLEAN");
			}
		}catch(SQLException ex){
			ex.printStackTrace();
		} finally {
